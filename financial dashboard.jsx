...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([{ role: "ai", text: "Hello! I'm your AI Financial Assistant. Ask me about cash flow, profit, recommendations, or upload a bank statement! 💰" }]);
...
Use realistic numbers based on the trend. Output only the JSON array.`, 600)
  ...
  const processStatement = async (file) => {
    ...
    try {
      setStmtProgress("Reading PDF..."); setStmtPct(20);
      const txt = await extractPdf(file);
      setStmtProgress("Claude AI extracting transactions..."); setStmtPct(55);
      let extracted = [];
      try {
        const raw = await callClaude(`Parse ALL transactions from this bank statement. Return ONLY a JSON array. Each object: {"date":"YYYY-MM-DD","description":"string","credit":number,"debit":number,"balance":number}. Remove commas from numbers. Cr=credit, Dr=debit. Use 2024 if year missing.\n\n${txt.slice(0, 14000)}`, 4000);
      ...
      } catch { setAiInsight("Statement processed successfully. Review transactions and use the date filter to analyze specific days."); }
      if (!Array.isArray(extracted) || extracted.length === 0) setAiInsight("Demo data shown — upload a text-based PDF for real extraction.");
      setStmtPct(100); setStmtStatus("done");
    } catch { setStmtTxns(DEMO_STMT()); setStmtPct(100); setStmtStatus("done"); setAiInsight("Demo data shown. Upload a text-based PDF for real extraction."); }
  };
  ...
  if (page === "login") return (
    <div style={{ ...S.app, display: "flex", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg, ${bg}, #dbeafe)`, minHeight: "100vh" }}>
      <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&display=swap" rel="stylesheet" />
      <div style={{ ...S.card, width: 420, padding: 40 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 52 }}>💹</div>
          <h1 style={{ fontSize: 28, fontWeight: 800, background: `linear-gradient(135deg, ${accent}, ${accentDark})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", margin: "8px 0 0" }}>FinanceIQ</h1>
          <p style={{ color: textMuted, fontSize: 14, margin: "4px 0 0" }}>Smart Financial Dashboard</p>
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 24, background: dark ? "#1a1f2e" : "#f1f5f9", borderRadius: 10, padding: 4 }}>
          {["login","signup"].map(m => <button key={m} onClick={() => setAuthMode(m)} style={{ flex: 1, padding: 8, border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 14, background: authMode === m ? accent : "transparent", color: authMode === m ? "#fff" : textMuted, transition: "all 0.2s" }}>{m === "login" ? "Login" : "Sign Up"}</button>)}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {authMode === "signup" && <>
            <input style={S.inp} placeholder="Full Name" value={loginData.name} onChange={e => setLoginData({ ...loginData, name: e.target.value })} />
            <select style={S.inp} value={loginData.business} onChange={e => setLoginData({ ...loginData, business: e.target.value })}>
              {["Café","Grocery Shop","Boutique","Freelancer","Bakery","Salon","Restaurant","Pharmacy","Electronics","Other"].map(b => <option key={b}>{b}</option>)}
            </select>
            <select style={S.inp} value={loginData.role} onChange={e => setLoginData({ ...loginData, role: e.target.value })}>
              <option value="owner">Owner</option><option value="manager">Manager</option><option value="accountant">Accountant</option>
            </select>
          </>}
          <input style={S.inp} type="email" placeholder="Email" value={loginData.email} onChange={e => setLoginData({ ...loginData, email: e.target.value })} />
          <input style={S.inp} type="password" placeholder="Password" value={loginData.password} onChange={e => setLoginData({ ...loginData, password: e.target.value })} />
          <button style={{ ...S.btn, width: "100%", padding: 14, fontSize: 16 }} onClick={() => { setUser({ name: loginData.name || "Business Owner", email: loginData.email, business: loginData.business, role: loginData.role }); setPage("app"); }}>
            {authMode === "login" ? "Login" : "Sign Up"} →
          </button>
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 20 }}>
          {["en","hi","gu"].map(l => <button key={l} onClick={() => setLang(l)} style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${border}`, background: lang === l ? accent : "transparent", color: lang === l ? "#fff" : textMuted, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>{l === "en" ? "EN" : l === "hi" ? "हि" : "ગુ"}</button>)}
        </div>
      </div>
    </div>
  );
      ...
      <div>
        ...
        {(stmtStatus === "idle" || stmtStatus === "error") && (
          <div onDragOver={e => { e.preventDefault(); setStmtDrag(true); }} onDragLeave={() => setStmtDrag(false)}
            ...
            style={{ border: `2px dashed ${stmtDrag ? accent : border}`, borderRadius: 20, padding: "64px 40px", textAlign: "center", cursor: "pointer", background: stmtDrag ? accent + "10" : cardBg, transition: "all 0.25s", marginBottom: 24, animation: "fadeIn 0.4s ease" }}>
            <input ref={fileRef} type="file" accept=".pdf" style={{ display: "none" }} onChange={e => e.target.files[0] && processStatement(e.target.files[0])} />
            <div style={{ fontSize: 72, marginBottom: 12 }}>📄</div>
            <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, color: stmtDrag ? accent : text }}>Drag & drop PDF here, or click to browse</div>
            <div style={{ color: textMuted, fontSize: 14, marginBottom: 24 }}>Works with HDFC, SBI, ICICI, Axis, Kotak, Yes Bank and more</div>
            <button style={{ ...S.btn, fontSize: 15, padding: "13px 36px", boxShadow: `0 6px 20px ${accent}50` }} onClick={e => { e.stopPropagation(); fileRef.current?.click(); }}>
              📤 Upload Bank Statement
            </button>
            <div style={{ marginTop: 24, display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
              {["🔒 Secure", "🤖 AI-Powered", "📊 Auto P&L", "⬇ CSV Export"].map(f => <div key={f} style={{ fontSize: 13, color: textMuted }}>{f}</div>)}
            </div>
          </div>
        )}
        ...
        {stmtStatus === "loading" && (
          <div style={{ ...S.card, textAlign: "center", padding: "64px 40px", marginBottom: 24 }}>
            ...
            <h3 style={{ margin: "20px 0 8px", fontWeight: 700, fontSize: 20 }}>Extracting transactions with AI...</h3>
            ...
            <div style={S.card}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: textMuted, marginTop: 2 }}>📄 {stmtFile?.name} · {stmtTxns.length} total</div>
                </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
          {[
            ...
            { icon: "📤", label: "Upload Statement", sub: "PDF bank statement", color: "#16a34a", action: () => setActiveNav("bankStatement") },
      ...
      <div style={S.sbar}>
        <div style={{ padding: "20px 14px", borderBottom: "1px solid rgba(255,255,255,0.15)", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {sidebarOpen && <div><div style={{ fontWeight: 800, fontSize: 17 }}>FinanceIQ</div><div style={{ fontSize: 10, opacity: 0.7 }}>{user?.business}</div></div>}
          </div>
        </div>
        ...
        {chatOpen && (
          <div style={{ ...S.card, width: 340, marginBottom: 12, maxHeight: 480, display: "flex", flexDirection: "column", boxShadow: `0 20px 60px ${accent}35`, animation: "slideUp 0.3s ease" }}>
            <div style={{ background: `linear-gradient(135deg, ${accent}, ${accentDark})`, padding: "13px 16px", borderRadius: "16px 16px 0 0", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 20 }}>🤖</span><div><div style={{ fontWeight: 700, fontSize: 14 }}>AI Assistant</div><div style={{ fontSize: 10, opacity: 0.75 }}>Powered by Claude</div></div></div>
